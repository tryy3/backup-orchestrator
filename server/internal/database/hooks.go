...
func (db *DB) CreateHook(h *PlanHook) error {
...
func (db *DB) GetHook(id string) (*PlanHook, error) {
	h := &PlanHook{}
	err := db.QueryRow(`
...
func (db *DB) ListHooks(planID string) ([]PlanHook, error) {
	rows, err := db.Query(`
		SELECT id, backup_plan_id, on_event, sort_order, script_id, type, command, timeout, on_error, created_at, updated_at
...
func (db *DB) UpdateHook(h *PlanHook) error {
...
func (db *DB) DeleteHook(id string) error {
	result, err := db.Exec("DELETE FROM plan_hooks WHERE id = ?", id)
	if err != nil {
...
func (db *DB) ReorderHooks(planID string, hookIDs []string) error {
	tx, err := db.Begin()
	if err != nil {
